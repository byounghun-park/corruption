...
# from .LiDAR_corruptions import gaussian_noise,lidar_crosstalk_noise,density_dec_global,\
#     density_dec_local,cutout_local,uniform_noise,\
#         background_noise,upsampling,impulse_noise,layer_del,\
#             scene_glare_noise,cutout_bbox,density_dec_bbox,gaussian_noise_bbox,scale_bbox,\
#                 shear_bbox,FFD_bbox,moving_noise_bbox,uniform_noise_bbox,\
#                     upsampling_bbox,impulse_noise_bbox,rotation_bbox,rain_sim,\
#                         fog_sim,snow_sim,spatial_alignment_noise,fulltrajectory_noise, \
#                             temporal_alignment_noise,fov_filter
from .LiDAR_corruptions import gaussian_noise,lidar_crosstalk_noise,density_dec_global,\
    cutout_local,uniform_noise,\
        impulse_noise,\
            scene_glare_noise,cutout_bbox,density_dec_bbox,gaussian_noise_bbox,scale_bbox,\
                shear_bbox,moving_noise_bbox,uniform_noise_bbox,\
                    impulse_noise_bbox,rotation_bbox,rain_sim,\
                        fog_sim,snow_sim,spatial_alignment_noise,fulltrajectory_noise, \
                            temporal_alignment_noise,fov_filter
...
class CorruptionMethods(object):
    ...
    #############################################
        ...
        if 'rain_sim_lidar' in self.corruption_severity_dict:
            ...
            # aug_pl = pl[:,:3]
            points_aug = rain_sim(pl.numpy(), severity)
            pl = torch.from_numpy(points_aug)
            results['points'].tensor = pl
        ...
        if 'fog_sim_lidar' in self.corruption_severity_dict:
            ...
            # aug_pl = pl[:,:3]
            points_aug = fog_sim(pl.numpy(), severity)
            pl = torch.from_numpy(points_aug)
            results['points'].tensor = pl
        ...
        if 'snow_sim_lidar' in self.corruption_severity_dict:
            ...
            # aug_pl = pl[:,:3]
            points_aug = snow_sim(pl.numpy(), severity)
            pl = torch.from_numpy(points_aug)
            results['points'].tensor = pl
